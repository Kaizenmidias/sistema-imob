<template>
  <AdminLayout>
    <template #pageTitle>Condomínios</template>
    ...
    <div v-if="showCreate" class="bg-white rounded-xl shadow p-6 border border-gray-200 mb-8">
      ...
      <form @submit.prevent="create" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div>
          ...
          <input ref="createNameRef" v-model="createForm.name" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-3" placeholder="Nome do condomínio">
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-if="items.length === 0">
            <td colspan="5" class="px-6 py-12 text-center text-gray-500">Nenhum condomínio cadastrado</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const create = () => {
  createForm.post(`${adminBase.value}/condominiums`, {
    onSuccess: () => {
      createForm.reset();
      createForm.clearErrors();
      showCreate.value = false;
...
const saveEdit = () => {
  ...
  editForm.put(`${adminBase.value}/condominiums/${editingId.value}`, {
    onSuccess: () => cancelEdit(),
  });
};

const remove = (id) => {
  router.delete(`${adminBase.value}/condominiums/${id}`);
};
</script>
